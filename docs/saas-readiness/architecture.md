...
## 13. Session/CSRF/rate-limit posture (T6, resolved from "unconfirmed")

- Session cookie flags are explicitly configured and reasonable: `httpOnly: true`, `secure: isProduction`, `sameSite: "lax"` (relaxed to `"none"`+`secure:true` only for a recognized Capacitor mobile-app origin in production), `maxAge` 24h, `domain: ".mypaylink.app"` in production, PG-backed session store (`connect-pg-simple`) (`server/index.ts:257-285`). This corrects the prior "not found" framing — the flags exist and are not obviously misconfigured.
- **No CSRF token mechanism exists anywhere** (`grep -rn "csrf" server/index.ts server/routes.ts` → zero hits). `sameSite: "lax"` provides partial CSRF mitigation for cross-site navigation but not for same-site or GET-triggered state changes.
- **Rate limiting exists, but only for platform diagnostics.** `server/diagnostics.ts:19,100` implements an in-memory (non-persistent, single-process) token-bucket limiter applied solely to `/api/admin/diagnostics/*` routes (`diagnostics.ts:193-195`). No rate limiting was found on `/api/trial/signup`, `/api/auth/login`, or `/api/demo/provision` (`grep -rln "rateLimit\|rate-limit\|express-rate-limit" server/ package.json` → only `diagnostics.ts`).

## 14. Public marketing site — trial-duration consistency (corrects a preliminary hypothesis)

A prior discovery pass hypothesized a 7-day vs. 14-day trial-duration conflict. Direct inspection found **no "7-day" mention anywhere** in `public-site/public/*.html` or `server/`. The actual, verified state: `server/routes.ts:23733`'s hardcoded 30-day trial (§9) and **every** marketing page — `index.html:655,762`, `features.html:433`, `demo.html:83`, `signup.html:7,108`, `pricing.html:7,42,171,202` — consistently say **30 days**, not 14. The real conflict is therefore *code+marketing (30 days, internally consistent with each other)* vs. *this program's target of a 14-day trial*, not an internal 7-vs-14 inconsistency. This is a product decision to make explicitly (see `launch-checklist.md`), not a bug to reconcile between two existing numbers.

## 15. Production tenancy, internal/complimentary billing, and platform-owner/tenant-owner separation — current state

This program requires the platform owner's own real businesses, already running on this platform, to be treated exactly like any other customer tenant for isolation, security, audit, entitlements, retention, and backups — plus an explicit, auditable internal/complimentary subscription concept, distinct from ordinary paid tenants, and a strict separation between platform-owner identity/permissions and tenant-owner/tenant-admin identity/permissions. None of this exists today:

- **No internal/complimentary billing concept in the schema.** `grep -n "billing_mode\|is_internal\|internal_subscription\|complimentary" shared/schema.ts` → zero hits. `companies.subscriptionStatus` (`shared/schema.ts:53`) defaults to `"active_paid"` — any company created outside the trial-signup path (which is how the owner's own pre-existing businesses would have been created, since `POST /api/trial/signup` did not always exist) starts, and remains, indistinguishable from a normal fully-paying customer. There is no `grantor`, `reason`, `effectiveDate`, `reviewDate`/`expirationDate`, or dedicated audit-event type for "this tenant's subscription is internal/complimentary."
- **Platform-owner access is not bounded away from tenant payroll/financial/document data today.** `expandRoleForGuard()` (`server/routes.ts:260-279`, §3) treats `platform_owner` identically to `platform_super_admin`/`platform_admin`: all three expand to `["admin", "manager", "supervisor", role]` and pass every `requireRole("admin", ...)`-guarded tenant route with no impersonation session, no reason, no expiry, no banner, and no audit event distinguishing "the platform owner operating a tenant" from "the platform owner supporting a customer tenant" from "a tenant's own admin." This is the specific case the program's platform-owner/tenant-owner separation requirement is meant to prevent, and it is not yet prevented — it is the same underlying mechanism as T1, applied specifically to the owner's own identity.
- **`platform_owner` is itself inconsistently recognized** — accepted by `expandRoleForGuard` (tenant-admin bypass, above) and by `diagnostics.ts`'s `DIAGNOSTIC_ROLES` (§3), but **absent from `requirePlatformRole()`'s allowlist** (`server/routes.ts:307-309`: `platform_super_admin, platform_admin, platform_sales, platform_implementation, platform_support, platform_billing, platform_auditor` — no `platform_owner`). A user with role `platform_owner` today would be rejected by the platform console's own tenant-management API (`requirePlatformRole()`-guarded) while simultaneously having full implicit tenant-admin access everywhere else and diagnostics access. This extends T2 with a concrete, resolvable instance.
- **No existing-production inventory or company→tenant mapping tooling exists.** The `tenants`/`tenant_companies` control-plane layer (§2b) is populated automatically only by new `POST /api/trial/signup` calls (`server/routes.ts:23768-23778`). Any company that predates this flow — which would include the owner's own real, already-operating businesses if they are managed through this codebase — has no `tenant_companies` row and is treated as "unassigned": `assertUserCanAccessCompany()` logs a warning and **allows** access rather than blocking (`server/tenant-context.ts:148-171`, §T3). No script, route, or report was found that enumerates existing `companies` rows, proposes a company→tenant mapping, or reconciles pre-existing records against the new tenant layer. Per this program's rules, no such mapping may auto-merge on shared owner/email/address — none of the existing code attempts to, but none of the required *safe, explicit* mapping tooling exists either.
- **No mechanism excludes a tenant's revenue from MRR/ARR while still recording its estimated commercial value.** Moot today only because MRR/ARR is not computed anywhere in the codebase at all (§11, gap-analysis §6) — this must be designed into Phase 3's revenue-reporting work from the start, not retrofitted after `tenant_subscriptions`/`billing_invoices` exist.

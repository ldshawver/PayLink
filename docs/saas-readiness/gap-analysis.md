...
| T2 | Three inconsistent platform-role allowlists coexist: `requirePlatformRole()`'s 7-role list, `requireSuperAdmin()`'s single `platform_super_admin` check, and `diagnostics.ts`'s `DIAGNOSTIC_ROLES = {"platform_owner","super_admin","system_admin"}` — none of the latter three strings appear in the other two lists. Depending on which strings are actually assigned to real users, this could mean either (a) intended platform-owner users are locked out of diagnostics, or (b) a role string exists that grants diagnostics access without being a recognized platform role elsewhere — undetermined without a users-table audit, which was out of scope for Phase 0 per the "no live database" rule. | `server/routes.ts:306-324,326-334`, `server/diagnostics.ts:8` | **HIGH — resolve before consolidating into the Phase 1 platform-owner model** |
...
| T6 | No CSRF token mechanism, session-cookie flag configuration (`httpOnly`/`secure`/`sameSite`), or login/signup/demo-provisioning rate limiting was found in the files read in this pass (`server/index.ts` session setup at line 257 onward was read only partially; a full audit of `express-session` config flags and any rate-limit middleware was not completed). Flag as **unconfirmed, not "absent"** — needs a dedicated Phase 7 pass. | `server/index.ts:257+` (partial read) | **MEDIUM (needs confirmation)** |

## 3. Billing / trial gaps

| # | Finding | Evidence | Severity |
|---|---|---|---|
| BL1 | Trial length is hardcoded to **30 days** at signup (`server/routes.ts:23733`), contradicting the product's own "14-Day Trial" framing used elsewhere (marketing copy, program brief). This is a one-line config fact, not a structural gap, but it will silently ship the wrong number if untouched. | `server/routes.ts:23733` | **LOW (but must be an explicit decision, not an oversight)** |

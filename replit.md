...
-   **Schedule Publishing:** Schedules are created as "draft" by default. Admins/managers can click the "Publish Schedule" button in the schedule grid toolbar to publish all draft shifts in a date range. Publishing changes status to "published" and sends email + SMS notifications to each employee/contractor on the schedule. Draft shifts show a dashed amber border and "DRAFT" label; published shifts show a solid green border. The Publish button displays a badge count of unpublished shifts in the current view. Notification services: email via Nodemailer (requires SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM env vars); SMS via Twilio (requires TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER env vars). If not configured, schedules still publish but notifications are skipped. Backend: `POST /api/schedules/publish` endpoint (requires admin/manager role).
...
-   **Nodemailer:** Email notifications for schedule publishing (optional; configure SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM).
-   **Twilio:** SMS notifications for schedule publishing (optional; configure TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER).
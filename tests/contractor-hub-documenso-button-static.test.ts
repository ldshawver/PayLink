...
ok("Documenso button explains missing signer requirement", page.includes("getDocumensoDisabledReason") && policy.includes("Add at least one signer with an email before sending via Documenso"));
...
ok("Signature actions support draft pending awaiting signatures", policy.includes('["draft", "pending", "awaiting_signatures"]'));

...
ok("deploy-app is staging-only", workflow.includes("Deploy MyPayLink Staging") && workflow.includes("APP_ENV: staging") && workflow.includes("PM2_PROCESS: paylink-staging") && !workflow.includes("- production"));
ok("push/main deploys staging only", workflow.includes("on:\n  push:\n    branches:\n      - main") && workflow.includes("Pushes to main always deploy staging."));
...
ok("workflow has no luxit or systemd targets", !workflow.includes("luxit") && !workflow.includes("/root/lux-email-bot") && !workflow.includes("systemctl") && !workflow.includes("systemd"));
